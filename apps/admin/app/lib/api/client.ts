  ...
  private setupInterceptors() {
    ...
    this.api.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        // TEMPORARY: return the real error so we can see upload issues on phone
        // Remove this block and restore the refresh logic once upload works
        if (status === 401) {
          return Promise.reject(this.normalizeError(error));
        }

        
        // ===== RESTORE THIS LATER (token refresh) =====
