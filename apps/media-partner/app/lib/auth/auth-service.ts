...
export class AuthService {
  ...
  async signUp(data: SignUpRequest): Promise<SignUpResponse> {
    ...
    formData.append('termsAndCondition', String(data.termsAndCondition));
    ...
    return apiClient.post<SignUpResponse>("/auth/media-partners/sign-up", formData);
  }
  ...
  async login(data: LoginRequest): Promise<LoginResponse> {
    const response = await apiClient.post<LoginResponse>("/auth/media-partners/login", data);
  ...
  async generateTokens(
    ...
    const response = await apiClient.post<GenerateTokensResponse>("/auth/media-partners/generate-tokens", data);
  ...
  async verifyEmail(token: string): Promise<{ message: string }> {
    return apiClient.post<{ message: string }>("/auth/media-partners/verify-email", { token });
  }
  ...
  async forgotPassword(data: ForgotPasswordRequest): Promise<{ message: string }> {
    return apiClient.post<{ message: string }>("/auth/media-partners/forget-password", data);
  }

  async resetPassword(data: ResetPasswordRequest): Promise<{ message: string }> {
    return apiClient.post<{ message: string }>("/auth/media-partners/reset-password", data);
  }
  ...
  async getCurrentUser(): Promise<Profile> {
    ...
    const response = await apiClient.get<{ status: string; data: { profile: Profile } }>("/auth/media-partners/me");
